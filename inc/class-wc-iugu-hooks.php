...
class WC_Iugu_Hooks2 {
	...
	public function woocommerce_subscription_can_date_be_updated($can_date_be_updated, $date_type, $subscription) {
		if (($date_type == 'cancelled') || ($date_type == 'end') || ($date_type == 'next_payment')) {
			$can_date_be_updated = $subscription->get_status() == 'pending-cancel';
		}
		return $can_date_be_updated;
	}

	function subscriptions_is_recurring_fee($is_recurring, $fee, $cart) {
		if ($fee->name == __('Bank Slip discount', 'iugu-woocommerce')) {
	...
	function my_custom_checkout_field_display_admin_order_meta($order) {
		echo '<p class="form-field"><strong>' . __('IUGU Customer ID', 'iugu-woocommerce') . ':</strong> ' . get_user_meta($order->get_user_id(), '_iugu_customer_id', true) . '</p>';
	}
	...
	public function product_iugu_payment_options() {
		...
		woocommerce_wp_select(
			array(
				...
				'class'       => 'wc_input_subscription_length select short',
				'label'       => __('Number of Installments', 'iugu-woocommerce'),
				'options'     => array(
					'01'			    => '01',
					'02'			    => '02',
					'03'			    => '03',
	...
	public function saves_product_iugu_payment_options($product_id) {
		...
		if ($product == 'product') {
			...
			update_post_meta($product_id, '_iugu_number_installments', $_POST['_iugu_number_installments']);
		} // end if;
	...
	public function filter_gateways_based_on_product($gateways) {
		...
		if ($order_id > 0) {
			...
			if ($_payment_method == 'iugu-credit-card' || $_payment_method == 'pre_orders_pay_later') {
				...
				if ($change_payment > 0) {
					...
					if (function_exists('wcs_get_early_renewal_url')) {
						...
						if ($renew_url != '') {
							$renew_url = '<b><a style="padding-left: 1em;" href="' . $renew_url . '">' . __('Click here to do this now.', 'iugu-woocommerce') . '</a></b>';
						}
					}
					$msg = sprintf(__('Use the "%s" option to change payment method.', 'iugu-woocommerce'), __('Renew now', 'iugu-woocommerce')) . $renew_url;
			...
			if ($change_payment > 0 && count($filtered_gateways) == 0) {
				...
				$msg = sprintf(__('Use the "%s" option for this signature.', 'iugu-woocommerce'), __('Renew now', 'iugu-woocommerce')) . $renew_url;
				if (!wc_has_notice($msg, 'notice')) {
					wc_add_notice($msg, 'notice');
				}
				$msg = __('ATTENTION - When changing the payment method, a new charge will be made.', 'iugu-woocommerce');
				if (!wc_has_notice($msg, 'notice')) {
					wc_add_notice($msg, 'notice');
				}
			}
	...
	public function add_amount_to_order($cart) {
		...
		foreach ($cart_fees as $item_id => $fee) {
			if (
				$fee->name != __('Fees', 'iugu-woocommerce') &&
				$fee->name != __('Bank Slip discount', 'iugu-woocommerce')
			) {
		...
		if ($chosen_payment_method === 'iugu-credit-card') {
			...
			if (
				...
				if ($fee_value > 0) {
					$cart->add_fee(__('Fees', 'iugu-woocommerce'), $fee_value);
				}
			}
		}
